  ...
  return (
    <div className="flex items-center p-3 border-b bg-background/95 backdrop-blur sticky top-0 z-10 gap-1">
      ...
      <div className="flex-1 min-w-0 ml-2">
        {/* Both lines truncate: a long shop name or product title would
            otherwise push the product thumbnail off a phone screen. */}
        <p className="font-semibold leading-tight truncate">{user.name}</p>
        <p className={cn('text-xs transition-colors leading-tight truncate', isTyping ? 'text-primary font-medium' : 'text-muted-foreground')}>
