...
export function SummaryStep({ onPrevStep, shippingAddress, paymentMethod, savedMethodId }: SummaryStepProps) {
  ...
  const { items, grandTotal, appliedCoupon, clearCart } = useCart();
  const { user } = useUser();
  ...
  const handleFinaliseOrder = async () => {
    ...
    try {
      // Get Firebase ID token to authenticate API calls
      ...
      if (!stripe) throw new Error('Stripe is not initialized. Please refresh.');
      ...
      if (paymentMethod === 'card' && elements) {
        ...
        // Saved card — payment method already attached to the intent
        confirmResult = await stripe.confirmCardPayment(clientSecret);
      }

      if (confirmResult.error) {
        throw new Error(confirmResult.error.message || 'Payment failed at confirmation step.');
      }
      ...
      const msg = error?.message || 'An unexpected error occurred. Your card was not charged.';
      setErrorMessage(msg);
      toast({ variant: 'destructive', title: 'Order Error', description: msg });
    } finally {
      setIsLoading(false);
    }
  };
