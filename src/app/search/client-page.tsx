...
/** Everything this page needs to read a filter from — satisfied by both
 *  `ReadonlyURLSearchParams` and the merged `URLSearchParams` the category
 *  routes build. */
type FilterParams = Pick<URLSearchParams, 'get'>;
...
// ─── Filter injection ─────────────────────────────────────────────────────────
//
// The clean category routes (/women/shirts) carry their filters in the path,
// not the query string, but every filter read below goes through
// `useSearchParams()`. Rather than thread props through four components, a
// context supplies "base" filters that are merged underneath the real query
// params — so `/women/shirts?color=black` still narrows correctly, and the
// query string wins on conflict.
const FilterOverrideContext = React.createContext<Record<string, string> | null>(null);

/** `useSearchParams()` with the route's own filters merged in beneath it. */
function useFilterParams(): URLSearchParams {
  const searchParams = useSearchParams();
  const overrides = React.useContext(FilterOverrideContext);
  return React.useMemo(() => {
    if (!overrides) return searchParams as unknown as URLSearchParams;
    const merged = new URLSearchParams();
    Object.entries(overrides).forEach(([k, v]) => { if (v) merged.set(k, v); });
    searchParams.forEach((v, k) => { if (v) merged.set(k, v); });
    return merged;
  }, [searchParams, overrides]);
}

/**
 * Results grid with filters supplied by the caller. Used by the category
 * routes; `/search` renders the same tree with no overrides.
 */
export function SearchResults({ overrides }: { overrides: Record<string, string> }) {
  return (
    <FilterOverrideContext.Provider value={overrides}>
    ...
    </FilterOverrideContext.Provider>
  );
}
        ...
        <div className="grid grid-cols-2 gap-3">
          {GENDERS.map((g) => (
            <Button
              ...
              onClick={() => router.push(buildCategoryPath(g.value))}
            >
              {g.label}
            </Button>
          ))}
